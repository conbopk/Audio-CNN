...
class AudioProcessor:
    ...
    def process_audio_chunk(self, audio_data):
        waveform = torch.from_numpy(audio_data).float()      # [N] -> 1D tensor

        waveform = waveform.unsqueeze(0)                     # -> [1, N]  (channel dim)

        spectrogram = self.transform(waveform)               # -> [1, n_mels, time]

        return spectrogram.unsqueeze(0)                      # -> [batch=1, channel=1, n_mels, time]
...
class AudioClassifier:
    ...
    def inference(self, request: InferenceRequest):
        ...
        if audio_data.ndim > 1:
            audio_data = np.mean(audio_data, axis=1)    # [num_samples]
